...
多模态 LoRA：同上流程，模型换 Qwen2-VL-7B，数据用 LLaMA-Factory 的图文混合格式（如 COCO 2017 抽样 + caption），其余脚本与卡数不变。

可选放大：

32B：Qwen2.5-32B，BF16-LoRA，4 卡峰值约 22 GB/卡
70B：Llama-3.1-70B，NF4-QLoRA，4 卡峰值约 20 GB/卡 记录显存峰值、训练速度 (tokens/s)、MT-bench 得分。
图像编辑模型和图像生成模型的学习与实践，待补充具体步骤。
